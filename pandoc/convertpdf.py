...
def batchPdfConversion(SourceFolder,DestinationFolder):
    ...
    for file in files:
        ...
        command = ['pandoc',"--variable","fontsize=14pt","--variable","documentclass=extarticle",os.path.join(SourceFolder,file),'--latex-engine=xelatex','--template=me.latex','-o',os.path.join(DestinationFolder,replaceMdByPdf(file))]
    ...
    for folder in folders:
        ...
        for file in filess:
            print("starting conversion: " + file + " to pdf...")
            command = ['pandoc',"--variable","fontsize=14pt","--variable","documentclass=extarticle",os.path.join(SourceFolder,folder,file),'--latex-engine=xelatex','--template=me.latex','--highlight-style=pygments','-o',os.path.join(DestinationFolder,folder,replaceMdByPdf(file))]
            subprocess.run(command)
            print("conversion completed: " + file + " to pdf...")
    ...
    for folder in folders:
        ...
        for filename in files:
            ...
            print("combined " + filename)
        print(sanitizeFoldername(folder) + "has been compiled. saving in " + os.path.join(DestinationFolder,folder))
